...
const bigBook = document.getElementById("big_book");
const producerContainer = document.getElementById("producer_container");
/**************
 *   SLICE 1
 **************/

function updateBookView(bookQty) {
  // Create a reference to the element who's ID is 'coffee_counter'
  const bookCounter = document.getElementById("book_counter");
  // Set the innerText of that element to be the coffeeQty passed into this function
  bookCounter.innerText = bookQty;
}

function clickBook(data) {
...
function unlockProducers(producers, bookCount){
     for (let i =0; i <producers.length; i++){
        let currentProduct = producers[i]
        if (bookCount >= currentProduct.price/2){
            currentProduct.unlocked = true;
        }
    }
}
...
function makeProducerDiv(producer) {
  ...
  const html = `
    ...
    <div class="producer-column">
      ...
      <div>Book/second: ${producer.cps}</div>
      <div>Cost: ${currentCost} Books</div>
    </div>
    `;
...
function renderProducers(data) {
  // call the unlockProducers function and pass it data.producers and data.coffee
    unlockProducers(data.producers, data.book);
...
function canAffordProducer(data, producerId) {
  return getProducerById(data, producerId).price <= data.book;
}
...
// the event listener should call the clickCoffee function, and pass in the global data object
    bigBook.addEventListener("click", function (){
        clickBook(data)
    })
